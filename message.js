...
module.exports = nsc = async (nsc, m, chatUpdate, store) => {
try {
...
const botNumber = await nsc.decodeJid(nsc.user.id);
...
//group
const groupMetadata = m?.isGroup ? await nsc.groupMetadata(m?.chat).catch(e => {}) : {};
...
╰───────────── ╼
`
nsc.sendMessage(m?.chat,{text:tek,mentions:[m?.sender]},{quoted:m})
}
break
...
await nsc.groupParticipantsUpdate(m?.chat, [users], 'add').catch(console.log)
}
break
...
let users = m?.quoted ? m?.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await nsc.groupParticipantsUpdate(m?.chat, [users], 'remove').catch(console.log)
}
break
...
let users = m?.quoted ? m?.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await nsc.groupParticipantsUpdate(m?.chat, [users], 'promote').catch(console.log)
}
break
...
let users = m?.quoted ? m?.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await nsc.groupParticipantsUpdate(m?.chat, [users], 'demote').catch(console.log)
}
break
...
let media = await nsc.downloadAndSaveMediaMessage(qmsg)
...
let img = await nsc.downloadAndSaveMediaMessage(q)
...
await nsc.sendImageAsSticker(m?.chat, randomStickers[i], m, {
...
nsc.sendMessage(m.chat,{image: krt.result, caption: "Done"},{quoted:m})
}
break
...
nsc.sendMessage(m.chat, { audio: buff, mimetype: 'audio/mpeg' }, { quoted : m })
fs.unlinkSync(ran)
})
...
if (data.img) return nsc.sendMessage(m?.chat, { image: data.img, caption: data.desc }, { quoted: m })
...
if (stiker) nsc.sendFile(m?.chat, stiker, 'wms.webp', '', m, false, { asSticker: true })
...
nsc.sendMessage(m?.chat, { audio: Buffer.from(a.audioUrl.split("base64,")[1],"base64"), mimetype: "audio/mpeg" })
}
break
...
nsc.sendFile(m?.chat, This, "", "Done", m);
}
break
...
nsc.sendMessage(m.chat, { video: { url: res.nowm }, caption: res.title, fileName: `tiktok.mp4`, mimetype: 'video/mp4' }).then(() => {
nsc.sendMessage(m.chat, { audio: { url: res.audio }, fileName: `tiktok.mp3`, mimetype: 'audio/mp4' })
})
}
break
...
if (!/text|json/.test(res.headers.get('content-type'))) return nsc.sendFile(m?.chat, url, 'file', `*Link:* ${linknyaurl}\n\n2024 © PutuOfc`, m)
...
if (/video/.test(type)) {
return nsc.sendFile(m?.chat, buffer, 'media.mp4', msg[type].caption || '', m)
} else if (/image/.test(type)) {
return nsc.sendFile(m?.chat, buffer, 'media.jpg', msg[type].caption || '', m)
}
}
break
...
let ppnyauser = await await nsc.profilePictureUrl(m?.sender, 'image').catch(_=> 'https://telegra.ph/file/6880771a42bad09dd6087.jpg')
const rest = await quote(text, pushname, ppnyauser)
nsc.sendImageAsSticker(m?.chat, rest.result, m, { packname: `${global.packname}`, author: `${global.author}`})
}
break
...
if (/image/.test(mime)) {
let media = await quoted.download()
let encmedia = await nsc.sendImageAsSticker(m?.chat, media, m, {
packname: global.packname,
author: global.author
})
...
let media = await quoted.download()
let encmedia = await nsc.sendVideoAsSticker(m?.chat, media, m, {
packname: global.packname,
author: global.author
})
await fs.unlinkSync(encmedia)
...
let dwnld = await nsc.downloadAndSaveMediaMessage(qmsg)
let fatGans = await TelegraPH(dwnld)
let smeme = `https://api.memegen.link/images/custom/${encodeURIComponent(bawah)}/${encodeURIComponent(atas)}.png?background=${fatGans}`
let FaTiH = await nsc.sendImageAsSticker(m?.chat, smeme, m, { packname: global.packname, author: global.auhor })
...
if (/image/.test(mime)) {
let media = await nsc.downloadMediaMessage(qmsg)
let encmedia = await nsc.sendImageAsSticker(m?.chat, media, m, { packname: teks1, author: teks2 })
await fs.unlinkSync(encmedia)
} else if (/video/.test(mime)) {
if ((quoted.msg || quoted).seconds > 11) return m?.reply('Maksimal 10 detik!')
let media = await nsc.downloadMediaMessage(qmsg)
let encmedia = await nsc.sendVideoAsSticker(m?.chat, media, m, { packname: teks1, author: teks2 })
await fs.unlinkSync(encmedia)
} else {
return m?.reply(`Kirim Gambar/Video Dengan Caption ${prefix + command}\nDurasi Video 1-9 Detik`)
}
...
try {
await new Promise(resolve => setTimeout(resolve, i * 6000));
nsc.sendMessage(m?.chat, { image: { url: imagesUrls[i] }, caption: `Image *(${i + 1}/${totalCount})*\n\nRemaining Credits: ${credits}\nPrompt: ${text}` }, { quoted: m });
} catch (error) {
console.error(`Error sending file: ${error.message}`);
...
await nsc.sendMessage(m?.chat, {react: {text: `🤨`,key: m?.key,}})
...
let caption = `Hᴀʟᴏ @${m?.sender.split('@')[0]} ${ucapanWaktu}, ᴀᴅᴀ ᴀᴘᴀ, Jɪᴋᴀ ᴘᴇɴᴛɪɴɢ ᴛɪɴɢɢᴀʟᴋᴀɴ ᴄʜᴀᴛ ᴅᴀɴ ᴋᴀᴍɪ ᴀᴋᴀɴ ᴍᴇᴍʙᴀʟᴀꜱ ꜱᴇᴄᴇᴘᴀᴛ ᴍᴜɴɢᴋɪɴ.`.trim();
nsc.sendMessage(m?.chat,{text:caption,mentions:[m?.sender]},{quoted:m})
user.pc = new Date() * 1;
}
}
} catch (err) {
nsc.sendMessage(global.owner, { text: util.format(err) })
}
}
